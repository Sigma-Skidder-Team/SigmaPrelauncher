...
public class JelloPrelauncher {
   ...
   public void setupRuntime() {
      ...
      if (!jreDir.exists()) {
         try {
            ...
            Utils.downloadFileFromUrl("https://github.com/MarkGG8181/cloud/raw/refs/heads/main/sigma/download/" + platform + "/jre", temporaryJreFile, (totalDownloadedSize, totalFileSize) -> this.df.setProgress((int)(100L * totalDownloadedSize / totalFileSize), "Updating Runtime"));
   ...
   private ArrayList<File> getFilesRecursive(File pFile) {
      ...
      // Check if the file is a directory and is not null
      if (pFile != null && pFile.isDirectory()) {
         File[] files = pFile.listFiles();

         // Ensure that files are not null (listFiles can return null in case of an error)
         if (files != null) {
            for (File file : files) {
               if (file.isDirectory()) {
                  ...
                  fileList.add(file);  // Add the file to the list
               }
            }
         } else {
   ...
   public void play() {
      ...
      (new Thread(() -> {
         ...
         launchGame(ressourceManager, jreBinLoc, macos, windows);
         System.exit(0);
      })).start();
   }
